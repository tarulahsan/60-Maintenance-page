...
const STORAGE_KEY = 'mp_target_09_framecraft';
const DEFAULT_DAYS = 18;
...
function setupEmailCapture() {
  ...
  form.addEventListener('submit', (e) => {
    ...
    try {
      const key = 'mp_emails_09_framecraft';
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  document.querySelectorAll('[data-reset]')?.forEach(btn => {
    btn.addEventListener('click', () => { resetCountdown(parseInt(btn.getAttribute('data-reset'), 10) || 18); window.scrollTo({ top: 0, behavior: 'smooth' }); });
  });
