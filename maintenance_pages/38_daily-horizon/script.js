...
const STORAGE_KEY = 'mp_target_38_daily-horizon';
...
function setupEmailCapture() {
  ...
  form.addEventListener('submit', (e) => {
    ...
    try {
      const key = 'mp_emails_38_daily-horizon';
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  document.querySelectorAll('[data-reset]')?.forEach(btn => {
    btn.addEventListener('click', () => { resetCountdown(parseInt(btn.getAttribute('data-reset'), 10) || 13); window.scrollTo({ top: 0, behavior: 'smooth' }); });
  });
