...
const STORAGE_KEY = 'mp_target_32_crafted-by-nova';
...
function setupEmailCapture() {
  ...
  form.addEventListener('submit', (e) => {
    ...
    try {
      const key = 'mp_emails_32_crafted-by-nova';
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  document.querySelectorAll('[data-reset]')?.forEach(btn => {
    btn.addEventListener('click', () => { resetCountdown(parseInt(btn.getAttribute('data-reset'), 10) || 24); window.scrollTo({ top: 0, behavior: 'smooth' }); });
  });
